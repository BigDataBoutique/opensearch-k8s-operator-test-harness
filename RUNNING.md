...
| Host disk shrinking over a long suite (tens of GB in a couple hours) | Docker build cache from repeated operator rebuilds + image churn from GC | `runner/oko_runner.py` now watches disk usage itself (`disk_watchdog`, runs automatically, nothing to arm): at `DISK_HIGH_PCT` (default 85, matching the threshold above) it pauses starting new playbooks and runs `docker builder prune -f`, resuming once usage drops below `DISK_LOW_PCT` (default 80). It never kills a running playbook or deletes a namespace — manual `docker builder prune` between suites still helps if you're not using the runner. |
| **Suite goes yellow-to-broken after several hours**: k3d nodes flap `NotReady`, cert-manager/operator pods `CrashLoopBackOff` on liveness timeouts, or playbooks fail with "no endpoints available for service" on a webhook | `/proc/pressure/io` saturates because **failed playbooks' namespaces pile up for hours** (`cleanup_on_failure: false`) with their clusters still running/indexing | The disk watchdog above catches the disk-usage side of this automatically during a `runner/oko_runner.py` run. It does *not* delete failed namespaces itself (`AUTO_CLEANUP_FAILED=1` still opts into that, see section 1); for a manual/long unattended run, run `poetry run oko-test cleanup` periodically (once failure evidence is collected — see section 3), or split into shorter batches. A genuine operator limitation can look identical to this pattern — always solo-rerun a suspect failure under a quiet host before writing it off as environmental. |
| Two playbooks racing `kubectl config use-context` fail instantly with `open ~/.kube/config.lock: file exists`; or one fails with `helm ... UPGRADE FAILED: release: already exists` while installing the operator for the first time | Two playbook processes hit the same transient lock/race at the same instant (every playbook is a separate process; the harness has no cross-process serialization for either) | Fixed as of 2026-09-23: `k8s.switch_context()` skips the write when already on the right context and retries the lock race; `install_operator`'s existing helm-busy retry now also covers `already exists`. If you still see either on an older checkout, just rerun the one playbook. |
| A migration/solo playbook fails with `cannot replace the operator while operator CRs exist: [...]` even though nothing else is running | An earlier solo/migration playbook in the same phase failed and left its `OpenSearchCluster` behind; solo playbooks run strictly sequentially with no cleanup between them (only once before the phase starts) | Fixed as of 2026-09-23: `run_solo()` now runs `poetry run oko-test free-crs <namespace>` after a solo playbook fails, deleting just its CR(s) (namespace/pods/events stay for inspection) so the next solo playbook isn't blocked. Manually: `poetry run oko-test free-crs <namespace>` (or the heavier `oko-test cleanup` if you also want the namespace gone). |
