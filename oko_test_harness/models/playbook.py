...
class Playbook:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Playbook":
        ...
        k8s_data = filter_dataclass_kwargs(
            KubernetesConfig, config_data.get("kubernetes", {})
        )
        ...
        opensearch_data_clean = {
            k: v for k, v in opensearch_data.items() if k != "security"
        }
        opensearch_filtered = filter_dataclass_kwargs(
            OpenSearchConfig, opensearch_data_clean
        )
        os_config = OpenSearchConfig(security=security_config, **opensearch_filtered)

        timeout_data = filter_dataclass_kwargs(
            TimeoutConfig, config_data.get("timeouts", {})
        )
        timeout_config = TimeoutConfig(**timeout_data)

        data_validation_data = filter_dataclass_kwargs(
            DataValidationConfig, config_data.get("data_validation", {})
        )
