...
class ScaleClusterAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            if not self._wait_for_scaling_completion(namespace, node_type, target_count, timeout):
                ...
                try:
                    with KubernetesOpenSearchClient(namespace=namespace) as client:
                        if not client.wait_for_cluster_health('green', 300):
                            return ActionResult(False, "Cluster did not reach green status after scaling")
                except Exception as e:
                    self.logger.warning(f"Could not verify cluster health: {e}")
            
            return ActionResult(True, f"Successfully scaled {node_type} nodes to {target_count}")
    ...
    def _wait_for_scaling_completion(self, namespace: str, node_type: str, 
                                   target_count: int, timeout: int) -> bool:
...
class ScaleDownClusterAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            if current_count <= target_count:
                return ActionResult(True, f"Already at or below target count ({current_count})")
            ...
            if drain_data and node_type == 'data':
                if not self._drain_data_nodes(namespace, current_count - target_count):
                    self.logger.warning("Data draining failed, proceeding with scale down")
            ...
            if not self._wait_for_scale_down_completion(namespace, node_type, target_count, timeout):
                return ActionResult(False, "Scale down operation timed out")
            
            return ActionResult(True, f"Successfully scaled down {node_type} nodes to {target_count}")
    ...
    def _wait_for_scale_down_completion(self, namespace: str, node_type: str, 
                                      target_count: int, timeout: int) -> bool:
