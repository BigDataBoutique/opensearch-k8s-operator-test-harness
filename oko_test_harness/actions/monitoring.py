...
class CollectLogsAction(BaseAction):
        ...
        try:
            ...
            for component in components:
                if component == 'operator':
                    ...
                    files = self._collect_opensearch_pod_logs(namespace, since, output_dir)
            ...
            return ActionResult(
                True, 
                f"Collected logs to {output_dir}: {len(collected_files)} files"
            )
            
        except Exception as e:
            return ActionResult(False, f"Failed to collect logs: {e}")
    
    def _collect_operator_logs(self, namespace: str, since: str, output_dir: str) -> list:
    ...
    def _collect_opensearch_pod_logs(self, namespace: str, since: str, output_dir: str) -> list:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for pod_name in pod_names:
                    if pod_name:
                        log_file = os.path.join(output_dir, f'opensearch-{pod_name}.log')
...
class CaptureMetricsAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            with KubernetesOpenSearchClient(namespace=namespace, service_name=service_name) as client:
                ...
                if duration_seconds > 60:  # Only do periodic capture for longer durations
                    interval = min(60, duration_seconds // 10)  # Capture 10 times or every minute
                    ...
                    for _ in range(duration_seconds // interval):
                        ...
                        for metric_type in metrics:
                            try:
                                ...
                                self.logger.warning(f"Failed to capture {metric_type}: {e}")
            ...
            return ActionResult(
                True,
                f"Captured metrics saved to {output_file}"
            )
...
class DebugPauseAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        print(f"Press Ctrl+C to abort, or Enter to continue immediately")
        print(f"{'='*60}")
