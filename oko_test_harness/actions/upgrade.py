...
class UpgradeClusterAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            if not current_version:
                return ActionResult(False, "Could not determine current cluster version")
            
            if strategy == 'rolling':
                return self._rolling_upgrade(
                    cluster_name, namespace, target_version, 
                    max_unavailable, validation_steps, rollback_on_failure, timeout_str
                )
    ...
    def _get_current_version(self, cluster_name: str, namespace: str) -> str:
        ...
        try:
            result = subprocess.run([
                ...
                '-o', 'jsonpath={.spec.general.version}'
    ...
    def _rolling_upgrade(self, cluster_name: str, namespace: str, target_version: str,
                        max_unavailable: int, validation_steps: List[str], 
                        rollback_on_failure: bool, timeout_str: str) -> ActionResult:
        ...
        try:
            ...
            import json
            result = subprocess.run([
                'kubectl', 'patch', 'opensearchcluster', cluster_name, '-n', namespace,
                '--type', 'merge', '-p', json.dumps(patch_data)
            ], capture_output=True, text=True)
            
            if result.returncode != 0:
                return ActionResult(False, f"Failed to update cluster spec: {result.stderr}")
            
            # Wait for upgrade to complete
            if not self._wait_for_upgrade_completion(cluster_name, namespace, target_version, timeout_str):
            ...
            return ActionResult(True, f"Successfully upgraded cluster to version {target_version}")
            
        except Exception as e:
            return ActionResult(False, f"Rolling upgrade failed: {e}")
    
    def _blue_green_upgrade(self, cluster_name: str, namespace: str, target_version: str) -> ActionResult:
    ...
    def _wait_for_upgrade_completion(self, cluster_name: str, namespace: str, 
                                   target_version: str, timeout_str: str) -> bool:
        """Wait for upgrade to complete."""
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if node_versions:
                    version_summary = ", ".join([f"{version}: {count} nodes" for version, count in node_versions.items()])
                    self.logger.info(f"Upgrade progress ({elapsed}s): {version_summary}")
                    ...
                    self.logger.info(f"Checking upgrade progress ({elapsed}s elapsed) - unable to get node versions")
    ...
    def _get_node_version_distribution(self, cluster_name: str, namespace: str) -> Dict[str, int]:
        ...
        try:
            ...
            result = subprocess.run([
                ...
                '-l', f'opster.io/opensearch-cluster={cluster_name}',
                '-o', 'jsonpath={range .items[*]}{.metadata.name}{";"}{.spec.containers[0].image}{";"}{.status.phase}{";"}{.status.containerStatuses[0].ready}{";"}{.status.containerStatuses[0].restartCount}{"\\n"}{end}'
            ], capture_output=True, text=True)
            ...
            for line in lines:
                ...
                if len(parts) >= 5:
                    ...
                    if ':' in image:
                        ...
                        version_key = f"{version}{status_indicator}" if status_indicator else version
    ...
    def _run_validation_step(self, step: str, namespace: str) -> bool:
        ...
        if step == 'check_cluster_health':
            ...
            for attempt in range(max_retries):
                try:
                    self.logger.info(f"Attempting to validate cluster health (attempt {attempt + 1}/{max_retries})")
                    
                    # Try to establish connection with retry
                    client = KubernetesOpenSearchClient(namespace=namespace, service_name=cluster_name)
                    if client.connect(quiet=True):
                        ...
                        self.logger.warning(f"Failed to connect to cluster on attempt {attempt + 1}")
...
class UpgradeOperatorAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            if strategy == 'recreate':
                ...
                return ActionResult(False, f"Unsupported operator upgrade strategy: {strategy}")
    ...
    def _recreate_upgrade(self, target_version: str, namespace: str, timeout_str: str) -> ActionResult:
        """Recreate operator with new version."""
        try:
            # Delete existing operator
            result = subprocess.run([
                'kubectl', 'delete', 'deployment', 'opensearch-operator', '-n', namespace
            ], capture_output=True, text=True)
            ...
            if result.returncode == 0:
                return ActionResult(True, f"Operator upgraded to version {target_version}")
            else:
                return ActionResult(False, f"Failed to install new operator version: {result.stderr}")
