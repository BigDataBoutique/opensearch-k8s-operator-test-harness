        ...
        try:
            ...
            while connection_attempts < max_connection_attempts:
                ...
                try:
                    ...
                    if client.connect(quiet=(connection_attempts < max_connection_attempts)):
                        self.logger.info("Successfully connected to OpenSearch cluster")
                        break
                    else:
                        if connection_attempts < max_connection_attempts:
                            self.logger.debug(f"Connection attempt {connection_attempts} failed, retrying in {interval}s...")
                            time.sleep(interval)
                except Exception as e:
                    if connection_attempts < max_connection_attempts:
                        self.logger.debug(f"Connection attempt {connection_attempts} failed: {e}, retrying in {interval}s...")
                        time.sleep(interval)
                    else:
                        raise
            
            if not client or not hasattr(client, 'port_forward_process') or not client.port_forward_process:
                return ActionResult(False, f"Failed to establish connection after {max_connection_attempts} attempts")
                ...
                try:
                    ...
                    if not self._health_status_reached(current_status, expected_status):
                        ...
                        if remaining_time > interval:
                            self.logger.info(f"Cluster health is {current_status}, waiting for {expected_status} ({remaining_time:.0f}s remaining)")
                            time.sleep(interval)
                            continue
                        else:
                            return ActionResult(False, f"Cluster did not reach {expected_status} status within {timeout_str} (final status: {current_status})")
                    ...
                    if check_indices:
                        ...
                        if expected_status == 'green' and (relocating_shards > 0 or unassigned_shards > 0):
                            return ActionResult(
                                False, 
                    ...
                    remaining_time = timeout - (time.time() - start_time)
                    if remaining_time > interval:
                        self.logger.info(f"Health check failed: {e}, retrying in {interval}s...")
                        time.sleep(interval)
                    else:
                        return ActionResult(False, f"Health validation failed: {e}")
            
            return ActionResult(False, f"Failed to validate cluster health: Timeout after {timeout_str}")
            ...
            with client:
                ...
                for index_pattern in indices:
                    try:
                        ...
                        total_docs += count
                        self.logger.debug(f"Index pattern '{index_pattern}': {count} documents")
                    except Exception as e:
                        self.logger.warning(f"Could not count documents in '{index_pattern}': {e}")
                ...
                for i, query_config in enumerate(sample_queries):
                    ...
                    self.logger.debug(f"Running sample query {i+1}: {query} on index pattern '{index_pattern}'")
                    
                    # Show index mapping for debugging
                    try:
                        mapping = client.get_index_mapping(index_pattern)
                        if mapping:
                            self.logger.debug(f"Index mapping for '{index_pattern}': {mapping}")
                    except Exception as e:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Could not refresh index {index_pattern}: {e}")
                    ...
                    if min_hits is not None and hits < min_hits:
                        ...
                        try:
                            sample_result = client.search(index_pattern, {"match_all": {}}, size=5)
                            sample_docs = [doc['_source'] for doc in sample_result.get('hits', {}).get('hits', [])]
                            self.logger.debug(f"Sample documents from index: {sample_docs}")
                        except Exception as e:
                            self.logger.warning(f"Could not retrieve sample documents: {e}")
                ...
                return ActionResult(
                    True,
                    f"Data integrity validated: {total_docs} total documents"
                )
...
class WaitForClusterReadyAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        self.logger.info(f"Waiting for cluster '{cluster_name}' to be ready in namespace '{namespace}'")
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                try:
                    ...
                    if client.connect(quiet=is_quiet):
                        try:
                            ...
                            for condition in conditions:
                                if 'cluster_health' in condition:
                                    ...
                                    if not self._health_status_reached(health['status'], required_status):
                                        self.logger.debug(f"Cluster health is {health['status']}, waiting for {required_status}")
                                    ...
                                    if condition['all_nodes_ready']:
                                        ...
                                        if not node_stats.get('nodes'):
                                            self.logger.info("No nodes found in cluster stats, waiting...")
                                            all_conditions_met = False
                                            break
                                    ...
                                    if condition['indices_ready']:
                                        ...
                                        if unassigned > 0:
                                            self.logger.info(f"Found {unassigned} unassigned shards, waiting...")
                                            all_conditions_met = False
                                            break
                            
                            if all_conditions_met:
                                return ActionResult(True, f"All readiness conditions met for cluster '{cluster_name}'")
                        ...
                        if is_quiet:
                            self.logger.debug(f"Connection attempt {attempt}/{max_attempts} failed, retrying...")
            ...
            return ActionResult(False, f"Timeout waiting for cluster readiness after {timeout_str}")
        ...
        try:
            ...
            if not pods:
                ...
                if all_pods:
                    ...
                    return ActionResult(False, f"No operator pods found in namespace '{namespace}' with any known selector. Available pods: {', '.join(pod_names)}")
                else:
                    # Check if namespace exists and if operator installation might have failed
                    try:
                        # Try to get namespace info
                        import subprocess
                        ns_result = subprocess.run(['kubectl', 'get', 'namespace', namespace], 
                                                 capture_output=True, text=True)
                        if ns_result.returncode != 0:
                            return ActionResult(False, f"Namespace '{namespace}' does not exist. Operator installation may have failed. Check the 'install_operator' step in your playbook.")
                        else:
                            return ActionResult(False, f"No operator pods found in namespace '{namespace}' (namespace exists but is empty). Operator installation may have failed. Check the 'install_operator' step in your playbook.")
                    except Exception:
                        return ActionResult(False, f"No operator pods found in namespace '{namespace}' (unable to verify namespace status)")
            
            # Check for crash loops in operator pods using the successful selector
            crash_check = k8s_manager.check_crash_loops(namespace, label_selector=used_selector, max_restart_count=max_restart_count)
            ...
            for pod in pods:
                ...
                if not pod['ready']:
                    return ActionResult(False, f"Operator pod '{pod['name']}' is not ready")
            ...
            if crash_check['warnings']:
                restart_counts = [f"{w['pod_name']}:{w['restart_count']}" for w in crash_check['warnings']]
                restart_info = f" (restarts: {', '.join(restart_counts)})"
            
            return ActionResult(True, f"Operator is running with {len(pods)} pod(s) (found using selector: {used_selector}){restart_info}")
...
class FixClusterReplicasAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        self.logger.info(f"Fixing replica settings to {target_replicas} for all indices")
        ...
        try:
            ...
            with client:
                ...
                for index_info in indices:
                    ...
                    if not settings:
                        self.logger.warning(f"Could not get settings for index '{index_name}'")
                        continue
                    ...
                    if current_replicas != target_replicas:
                        self.logger.info(f"Index '{index_name}' has {current_replicas} replicas, updating to {target_replicas}")
                        if client.update_index_replicas(index_name, target_replicas):
                            fixed_count += 1
                        else:
                            self.logger.warning(f"Failed to update replicas for index '{index_name}'")
                    else:
                        self.logger.info(f"Index '{index_name}' already has {target_replicas} replicas")
...
class ValidateNodeConfigurationAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        namespace = params.get('namespace', self.config.opensearch.operator_namespace)
        node_group = params.get('node_group', 'all')  # 'all', 'master', 'data', 'ingest'
        ...
        try:
            ...
            if node_group == 'all':
                label_selector = "app.kubernetes.io/component in (opensearch-cluster,opensearch)"
            else:
            ...
            if not pods:
                return ActionResult(False, f"No OpenSearch pods found for node group '{node_group}'")
            ...
            for pod in pods:
                ...
                if vm_max_map_count is not None:
                    result = self._validate_vm_max_map_count(k8s_manager, namespace, pod_name, vm_max_map_count)
                    validation_results.append(result)
                    if not result['success']:
                        all_passed = False
                
                if vm_swappiness is not None:
                    result = self._validate_vm_swappiness(k8s_manager, namespace, pod_name, vm_swappiness)
                    validation_results.append(result)
                    if not result['success']:
                        all_passed = False
                
                if fs_file_max is not None:
                    result = self._validate_fs_file_max(k8s_manager, namespace, pod_name, fs_file_max)
                    validation_results.append(result)
                    if not result['success']:
                        all_passed = False
                ...
                if storage_checks:
                    result = self._validate_storage_configuration(k8s_manager, namespace, pod_name, storage_checks)
                    validation_results.append(result)
                    if not result['success']:
                        all_passed = False
                ...
                if jvm_heap_checks:
                    result = self._validate_jvm_heap_configuration(k8s_manager, namespace, pod_name, jvm_heap_checks)
                    validation_results.append(result)
                    if not result['success']:
                        all_passed = False
            ...
            return ActionResult(all_passed, summary, {'validation_details': validation_results})
            
        except Exception as e:
            return ActionResult(False, f"Failed to validate node configuration: {e}")
    
    def _validate_vm_max_map_count(self, k8s_manager: KubernetesManager, namespace: str, pod_name: str, expected_min: int) -> Dict[str, Any]:
    ...
    def _validate_vm_swappiness(self, k8s_manager: KubernetesManager, namespace: str, pod_name: str, expected_max: int) -> Dict[str, Any]:
    ...
    def _validate_fs_file_max(self, k8s_manager: KubernetesManager, namespace: str, pod_name: str, expected_min: int) -> Dict[str, Any]:
    ...
    def _validate_memory_configuration(self, pod: Dict[str, Any], memory_checks: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'min_memory_request' in memory_checks:
                ...
                if not memory_request:
                    validation_errors.append(f"No memory request specified, minimum required: {min_req}")
                elif not self._compare_memory_size(memory_request, min_req, ">="):
                    validation_errors.append(f"Memory request {memory_request} is less than minimum {min_req}")
            
            # Check memory limit
            if 'min_memory_limit' in memory_checks:
                min_limit = memory_checks['min_memory_limit']
                if not memory_limit:
                    validation_errors.append(f"No memory limit specified, minimum required: {min_limit}")
                elif not self._compare_memory_size(memory_limit, min_limit, ">="):
                    validation_errors.append(f"Memory limit {memory_limit} is less than minimum {min_limit}")
            
            # Check request/limit ratio
            if 'request_limit_ratio' in memory_checks and memory_request and memory_limit:
    ...
    def _validate_cpu_configuration(self, pod: Dict[str, Any], cpu_checks: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'min_cpu_request' in cpu_checks:
                ...
                if not cpu_request:
                    validation_errors.append(f"No CPU request specified, minimum required: {min_req}")
                elif not self._compare_cpu_size(cpu_request, min_req, ">="):
                    validation_errors.append(f"CPU request {cpu_request} is less than minimum {min_req}")
            
            # Check CPU limit
            if 'min_cpu_limit' in cpu_checks:
                min_limit = cpu_checks['min_cpu_limit']
                if not cpu_limit:
                    validation_errors.append(f"No CPU limit specified, minimum required: {min_limit}")
                elif not self._compare_cpu_size(cpu_limit, min_limit, ">="):
                    validation_errors.append(f"CPU limit {cpu_limit} is less than minimum {min_limit}")
    ...
    def _validate_storage_configuration(self, k8s_manager: KubernetesManager, namespace: str, pod_name: str, storage_checks: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_jvm_heap_configuration(self, k8s_manager: KubernetesManager, namespace: str, pod_name: str, jvm_checks: Dict[str, Any]) -> Dict[str, Any]:
        """Validate JVM heap configuration."""
        try:
            ...
            if not java_opts and 'require_heap_settings' in jvm_checks and jvm_checks['require_heap_settings']:
                ...
                if 'min_heap_size' in jvm_checks:
                    ...
                    if not self._check_jvm_heap_setting(java_opts, 'Xms', min_heap, '>='):
                        validation_errors.append(f"JVM initial heap size is less than minimum {min_heap}")
                
                if 'max_heap_size' in jvm_checks:
                    max_heap = jvm_checks['max_heap_size']
                    if self._check_jvm_heap_setting(java_opts, 'Xmx', max_heap, '>'):
                        validation_errors.append(f"JVM maximum heap size exceeds maximum {max_heap}")
    ...
    def _check_jvm_heap_setting(self, java_opts: str, setting: str, expected: str, operator: str) -> bool:
...
class ValidateClusterConfigurationAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        retry_interval_str = params.get('retry_interval', '30s')  # Retry every 30 seconds
        ...
        self.logger.info(f"Validating cluster configuration (timeout: {timeout_str}, retry interval: {retry_interval_str})")
                ...
                try:
                    ...
                    with client:
                        ...
                        if node_role_distribution:
                            result = self._validate_node_role_distribution(client, node_role_distribution)
                            validation_results.append(result)
                            if not result['success']:
                                all_passed = False
                    ...
                    if remaining_time > retry_interval:
                        ...
                        self.logger.debug(f"Validation checks failed: {failed_checks}. Retrying in {retry_interval_str} ({remaining_time:.0f}s remaining)")
                    ...
                    remaining_time = timeout - (time.time() - start_time)
                    if remaining_time > retry_interval:
                        self.logger.info(f"Validation error: {e}. Retrying in {retry_interval_str} ({remaining_time:.0f}s remaining)")
            ...
            for result in validation_results:
                if result['success']:
                    ...
                    if 'role_counts' in result:
                        self.logger.error(f"  Current role counts: {result['role_counts']}")
    ...
    def _validate_cluster_settings(self, client: KubernetesOpenSearchClient, cluster_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for setting_name, expected_config in cluster_settings.items():
                ...
                current_value = self._extract_setting_value(current_settings, setting_name)
                
                if current_value is None:
                    validation_errors.append(f"Setting '{setting_name}' is not configured")
                    continue
                ...
                if validation_type == 'equals':
                    ...
                    if minimum is not None and self._compare_numeric_values(current_value, minimum, '<'):
                        validation_errors.append(
                            f"Setting '{setting_name}' is {current_value}, minimum required {minimum}"
                        )
                elif validation_type == 'maximum':
                    if maximum is not None and self._compare_numeric_values(current_value, maximum, '>'):
                        validation_errors.append(
    ...
    def _validate_index_settings(self, client: KubernetesOpenSearchClient, index_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for index_info in indices:
                ...
                settings = client.get_index_settings(index_name)
                if not settings:
                    validation_errors.append(f"Failed to get settings for index {index_name}")
                    continue
                
                index_config = settings.get(index_name, {}).get('settings', {}).get('index', {})
                ...
                for setting_name, expected_config in index_settings.items():
                    if isinstance(expected_config, dict):
                        expected_value = expected_config.get('value')
                        validation_type = expected_config.get('type', 'equals')
                        apply_to = expected_config.get('apply_to', 'all')  # 'all', 'user', 'system'
                    else:
                        expected_value = expected_config
                        validation_type = 'equals'
                        apply_to = 'all'
    ...
    def _validate_node_role_distribution(self, client: KubernetesOpenSearchClient, role_distribution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not cluster_stats or 'nodes' not in cluster_stats:
                ...
                try:
                    ...
                    if node_info and 'nodes' in node_info:
                        debug_info += f", but found {len(node_info['nodes'])} nodes via nodes API"
                except:
            ...
            for role, requirements in role_distribution.items():
                ...
                if role == 'master' and role not in role_counts and 'cluster_manager' in role_counts:
            ...
            if validation_errors:
                ...
                for node in node_details:
                    roles_str = ", ".join(node['roles']) if node['roles'] else "no roles"
                    node_info_str += f"  - {node['name']} (ID: {node['id']}): [{roles_str}]\n"
    ...
    def _validate_security_settings(self, client: KubernetesOpenSearchClient, security_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if security_enabled and not client.using_auth():
                    validation_errors.append("Security is supposed to be enabled but cluster accepts unauthenticated requests")
                
            except Exception:
                # If we get an auth error, security might be properly enabled
                if not security_enabled:
                    validation_errors.append("Security appears to be enabled but was expected to be disabled")
            ...
            if ssl_enabled is not None:
                ...
                if ssl_enabled and not client.using_ssl():
                    validation_errors.append("SSL is required but client is not using HTTPS")
                elif not ssl_enabled and client.using_ssl():
                    validation_errors.append("SSL is disabled but client is using HTTPS")
    ...
    def _validate_plugin_settings(self, client: KubernetesOpenSearchClient, plugin_settings: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                for plugin in required_plugins:
                    if plugin not in installed_plugins:
                        validation_errors.append(f"Required plugin '{plugin}' is not installed")
                
                # Check forbidden plugins
                for plugin in forbidden_plugins:
                    if plugin in installed_plugins:
                        validation_errors.append(f"Forbidden plugin '{plugin}' is installed")
                
                # Check plugin-specific settings
                plugin_configs = plugin_settings.get('plugin_configs', {})
                for plugin, config in plugin_configs.items():
                    if plugin not in installed_plugins:
                        validation_errors.append(f"Cannot validate config for plugin '{plugin}' - not installed")
                        continue
    ...
    def _extract_setting_value(self, settings: Dict[str, Any], setting_path: str) -> Any:
...
class ValidateNetworkConfigurationAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            if port_accessibility:
                result = self._validate_port_accessibility(namespace, service_name, port_accessibility)
                validation_results.append(result)
                if not result['success']:
                    all_passed = False
            ...
            if external_connectivity:
                result = self._validate_external_connectivity(namespace, service_name, external_connectivity)
                validation_results.append(result)
                if not result['success']:
                    all_passed = False
    ...
    def _validate_port_accessibility(self, namespace: str, service_name: str, port_config: Dict[str, Any]) -> Dict[str, Any]:
        """Validate port accessibility."""
        try:
            validation_errors = []
            ...
            for required_port in required_ports:
                ...
                for service_port in service_ports:
                    if service_port.get('port') == port_num and service_port.get('protocol', 'TCP') == protocol:
                        port_found = True
                        break
                
                if not port_found:
                    validation_errors.append(f"Required port {port_num}/{protocol} is not exposed by service")
            ...
            for forbidden_port in forbidden_ports:
                ...
                for service_port in service_ports:
                    if service_port.get('port') == port_num:
                        validation_errors.append(f"Forbidden port {port_num} is exposed by service")
            ...
            if validation_errors:
                ...
                return {
                    'success': True,
                    'check': 'port_accessibility',
                    'message': f"All port accessibility requirements are met",
                    'service_ports': service_ports
                }
    ...
    def _validate_inter_node_connectivity(self, namespace: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                client = KubernetesOpenSearchClient.from_security_config(self.config.opensearch.security, namespace)
                with client:
                    cluster_stats = client.cluster_stats()
                    if cluster_stats and 'nodes' in cluster_stats:
                        node_count = cluster_stats['nodes'].get('count', {}).get('total', 0)
                        if node_count > 1:
    ...
    def _validate_external_connectivity(self, namespace: str, service_name: str, external_config: Dict[str, Any]) -> Dict[str, Any]:
        """Validate external connectivity."""
        try:
            validation_errors = []
            
            # Check if external access is required
            require_external_access = external_config.get('require_external_access', False)
            load_balancer_type = external_config.get('load_balancer_type')  # 'NodePort', 'LoadBalancer', 'Ingress'
            ...
            if require_external_access:
                if service_type == 'ClusterIP':
                    validation_errors.append("External access required but service type is ClusterIP")
                elif load_balancer_type and service_type != load_balancer_type:
                    validation_errors.append(f"Service type is {service_type}, expected {load_balancer_type}")
    ...
    def _validate_dns_resolution(self, namespace: str, service_name: str) -> Dict[str, Any]:
        """Validate DNS resolution."""
        try:
                ...
                with client:
                    ...
                    if health:
                        ...
                        return {
                            'success': False,
                            'check': 'dns_resolution',
                            'message': f"DNS resolves but service is not responding"
                        }
            except Exception as e:
                if "Name or service not known" in str(e) or "not resolve hostname" in str(e):
                    return {
                        'success': False,
                        'check': 'dns_resolution',
