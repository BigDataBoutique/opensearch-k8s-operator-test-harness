...
from oko_test_harness.utils.kubernetes import KindClusterManager, MinikubeClusterManager, KubernetesManager
...
class SetupClusterAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        k8s_version = params.get('kubernetes_version', self.config.kubernetes.kubernetes_version)
        ...
        if provider == "kind":
            return self._setup_kind_cluster(cluster_name, nodes, k8s_version, extra_config)
        elif provider == "minikube":
            return self._setup_minikube_cluster(cluster_name, nodes, k8s_version, extra_config)
        elif provider == "existing":
            return self._setup_existing_cluster()
        else:
            return ActionResult(False, f"Unsupported provider: {provider}")
    
    def _setup_kind_cluster(self, cluster_name: str, nodes: int, k8s_version: str, 
                           extra_config: str = None) -> ActionResult:
        ...
        if kind_manager.create_cluster(cluster_name, config_content):
            ...
            return ActionResult(True, f"Successfully created Kind cluster {cluster_name}")
        else:
            return ActionResult(False, f"Failed to create Kind cluster {cluster_name}")
    
    def _setup_minikube_cluster(self, cluster_name: str, nodes: int, k8s_version: str, 
                               extra_config: str = None) -> ActionResult:
        ...
        if minikube_manager.cluster_exists(cluster_name):
            ...
            if status == "Running":
                self.logger.info(f"Minikube cluster {cluster_name} already exists and is running")
                return ActionResult(True, f"Cluster {cluster_name} already exists")
            else:
                self.logger.info(f"Minikube cluster {cluster_name} exists but is not running, starting it...")
        
        if minikube_manager.create_cluster(cluster_name, nodes, k8s_version, extra_config):
            # Wait for cluster to be ready
            time.sleep(15)
            ...
            return ActionResult(True, f"Successfully created Minikube cluster {cluster_name}")
        else:
            return ActionResult(False, f"Failed to create Minikube cluster {cluster_name}")
    ...
    def _prepull_common_images(self, cluster_name: str, provider: str = "kind") -> None:
        ...
        for image in common_images:
            try:
                ...
                check_result = subprocess.run(["docker", "image", "inspect", image], 
                ...
                if provider == "kind":
                    subprocess.run(["kind", "load", "docker-image", image, "--name", cluster_name], 
                                 check=True, capture_output=True, text=True)
                elif provider == "minikube":
                    # For minikube, use minikube image load command
                    subprocess.run(["minikube", "image", "load", image, "--profile", cluster_name], 
                                 check=True, capture_output=True, text=True)
...
class InstallOperatorAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        helm_repo_url = params.get('helm_repo_url', 'https://opensearch-project.github.io/opensearch-k8s-operator/')
        local_chart_path = params.get('local_chart_path')  # For development versions
        git_chart_repo = params.get('git_chart_repo')      # For development versions from git
        ...
        if local_operator_path and local_operator_path.strip():
            self.logger.info(f"Installing OpenSearch operator from local source: {local_operator_path}")
            return self._install_local_operator(local_operator_path, namespace, wait_timeout)
        ...
        if local_chart_path or git_chart_repo:
            return self._install_dev_helm_chart(local_chart_path, git_chart_repo, git_chart_branch, 
                                               version, namespace, values_file, wait_timeout, self.config.opensearch.cluster_name)
        elif helm_chart:
            return self._install_via_helm(helm_chart, helm_repo_url, version, namespace, values_file, wait_timeout)
        else:
            return self._install_via_kubectl(version, namespace)
    
    def _install_via_helm(self, chart: str, repo_url: str, version: str, namespace: str, 
                         values_file: str = None, timeout: str = "5m") -> ActionResult:
        ...
        try:
            ...
            repo_add_result = subprocess.run(['helm', 'repo', 'add', repo_name, repo_url], 
                                           capture_output=True, text=True, timeout=30)
            
            if repo_add_result.returncode != 0 and "already exists" not in repo_add_result.stderr:
                return ActionResult(False, f"Failed to add Helm repo: {repo_add_result.stderr}")
            
            # Update repositories
            self.logger.info("Updating Helm repositories...")
            update_result = subprocess.run(['helm', 'repo', 'update'], 
                                         capture_output=True, text=True, timeout=60)
            
            if update_result.returncode != 0:
                return ActionResult(False, f"Failed to update Helm repos: {update_result.stderr}")
            ...
            return ActionResult(False, f"Error installing OpenSearch operator via Helm: {e}")
    
    def _install_dev_helm_chart(self, local_chart_path: str, git_chart_repo: str, 
                               git_chart_branch: str, version: str, namespace: str,
                               values_file: str = None, timeout: str = "5m", release_name: str = "opensearch-cluster") -> ActionResult:
        ...
        try:
            ...
            if git_chart_repo and not local_chart_path:
                ...
                self.logger.info(f"Cloning development chart from {git_chart_repo} (branch: {git_chart_branch})")
                
                clone_cmd = ['git', 'clone', '--branch', git_chart_branch, '--depth', '1', 
                           git_chart_repo, temp_dir]
                result = subprocess.run(clone_cmd, capture_output=True, text=True)
                
                if result.returncode != 0:
                    return ActionResult(False, f"Failed to clone chart repository: {result.stderr}")
                ...
                if not chart_path:
                    return ActionResult(False, "Could not find Chart.yaml in the cloned repository")
            ...
            if result.returncode == 0:
                ...
                return ActionResult(True, f"Development chart installed successfully from {chart_path}")
            else:
    ...
    def _install_via_kubectl(self, version: str, namespace: str) -> ActionResult:
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.debug("OpenSearch operator installed successfully via kubectl")
    ...
    def _install_local_operator(self, local_path: str, namespace: str, timeout: str = "5m") -> ActionResult:
        ...
        try:
            ...
            if not os.path.exists(local_path):
                return ActionResult(False, f"Local operator path does not exist: {local_path}")
            ...
            has_dockerfile = any(os.path.exists(os.path.join(local_path, f)) for f in required_files)
            has_go = any(os.path.exists(os.path.join(local_path, f)) for f in go_files)
            
            if not (has_dockerfile and has_go):
                return ActionResult(False, f"Local path does not appear to be an OpenSearch operator repository (missing Dockerfile, Makefile, or Go files): {local_path}")
            ...
            try:
                ...
                kind_cluster_result = subprocess.run(['kubectl', 'config', 'current-context'], 
                                                   capture_output=True, text=True)
                is_kind_cluster = kind_cluster_result.returncode == 0 and 'kind-' in kind_cluster_result.stdout
                
                if is_kind_cluster:
                    return self._install_local_operator_kind(local_path, namespace, timeout)
                else:
                    return self._install_local_operator_standard(local_path, namespace, timeout)
    ...
    def _install_local_operator_kind(self, local_path: str, namespace: str, timeout: str) -> ActionResult:
        ...
        try:
            ...
            self.logger.debug(f"Building operator image for Kind cluster: {cluster_name}")
            
            # Build the operator image
            build_result = subprocess.run(['make', 'docker-build', f'IMG={image_tag}'], 
                                        capture_output=True, text=True, timeout=600)
            
            if build_result.returncode != 0:
                return ActionResult(False, f"Failed to build operator image: {build_result.stderr}")
            
            self.logger.info("Loading operator image into Kind cluster...")
            
            # Load image into Kind cluster
            load_result = subprocess.run(['kind', 'load', 'docker-image', image_tag, '--name', cluster_name], 
                                       capture_output=True, text=True, timeout=300)
            
            if load_result.returncode != 0:
                return ActionResult(False, f"Failed to load image into Kind: {load_result.stderr}")
            
            # Deploy the operator
            self.logger.info("Deploying operator to cluster...")
            deploy_result = subprocess.run(['make', 'deploy', f'IMG={image_tag}', f'NAMESPACE={namespace}'], 
                                         capture_output=True, text=True, timeout=300)
            
            if deploy_result.returncode != 0:
                return ActionResult(False, f"Failed to deploy operator: {deploy_result.stderr}")
    ...
    def _install_local_operator_standard(self, local_path: str, namespace: str, timeout: str) -> ActionResult:
        ...
        try:
            ...
            build_push_result = subprocess.run(['make', 'docker-build-push', f'IMG={image_tag}'], 
                                             capture_output=True, text=True, timeout=600)
            ...
            if build_push_result.returncode != 0:
                ...
                if push_result.returncode != 0:
                    self.logger.warning(f"Failed to push image {image_tag}, assuming local registry: {push_result.stderr}")
...
class DeployClusterAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        nodes = params.get('nodes', {'master': 1, 'data': 1})  # Minimal defaults for Kind
        node_pools = params.get('node_pools', [])
        storage = params.get('storage', {'class': 'standard', 'size': '20Gi'})  # Smaller default
        ...
        if security_config is None:
            ...
            self.logger.info(f"Using default security config: enabled={security_config['enabled']}, username={security_config['username']}")
        ...
        dashboards_config = params.get('dashboards', {'enabled': False})  # Disable dashboards by default
        ...
        if security_config.get('enabled', False):
            self.logger.info(f"Security is enabled, creating security secrets for namespace {namespace}")
            if not self._create_security_secrets(k8s_manager, namespace, security_config):
        ...
        if template_file:
            ...
            manifest = self._generate_cluster_manifest(cluster_name, namespace, version, 
                                                     nodes, node_pools, storage, security_config, dashboards_config, cluster_settings)
        
        # Apply the manifest
        if k8s_manager.apply_yaml_manifest(manifest, namespace):
            return ActionResult(True, f"OpenSearch cluster {cluster_name} deployed successfully")
        else:
            return ActionResult(False, f"Failed to deploy OpenSearch cluster {cluster_name}")
    
    def _generate_cluster_manifest(self, name: str, namespace: str, version: str,
                                 nodes: Dict[str, int], node_pools: List[Dict[str, Any]], 
                                 storage: Dict[str, str], security_config: Dict[str, Any],
                                 dashboards_config: Dict[str, Any], cluster_settings: Dict[str, Any] = None) -> str:
  ...
  general:
    ...
    additionalConfig:"""
      ...
      "{setting_key}": "{setting_value}" """
        ...
        if security_enabled:
            admin_secret_name = security_config.get('admin_secret_name', 'admin-credentials-secret')
            security_secret_name = security_config.get('security_config_secret_name', 'securityconfig-secret')
            manifest += f"""
  security:
    config:
...
"""
    ...
    def _create_security_secrets(self, k8s_manager: KubernetesManager, namespace: str, 
...
"""
            ...
            if not k8s_manager.apply_yaml_manifest(admin_secret_manifest, namespace):
                self.logger.error(f"Failed to create admin credentials secret {admin_secret_name}")
                return False
...
'''
            ...
            for filename, content in security_config_yaml.items():
                security_secret_data[filename] = base64.b64encode(content.encode()).decode()
...
"""
            ...
            if not k8s_manager.apply_yaml_manifest(security_secret_manifest, namespace):
                self.logger.error(f"Failed to create security config secret {security_secret_name}")
                return False
    ...
    def _fix_cluster_allocation_settings(self, cluster_name: str, namespace: str,
                                       security_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info("Waiting for cluster to be accessible before fixing allocation settings...")

            for attempt in range(max_retries):
                try:
                    # Check if cluster is accessible (only check for successful response, ignore health status)
                    check_cmd = [
                        'kubectl', 'exec', f'{cluster_name}-nodes-0', '-n', namespace, '--',
                    ...
                    if result.returncode == 0:
                        self.logger.info(f"Cluster is accessible (attempt {attempt + 1})")
                        break
                    else:
                        self.logger.debug(f"Cluster not yet accessible (attempt {attempt + 1}/{max_retries}), waiting {retry_delay}s...")
                        time.sleep(retry_delay)

                except subprocess.TimeoutExpired:
                    self.logger.debug(f"Timeout checking cluster accessibility (attempt {attempt + 1}/{max_retries})")
                    time.sleep(retry_delay)

            else:
                self.logger.error("Cluster did not become accessible within timeout period")
                return False
            ...
            result = subprocess.run(get_settings_cmd, capture_output=True, text=True, timeout=15)
            ...
            try:
                ...
                current_enable = settings.get('transient', {}).get('cluster', {}).get('routing', {}).get('allocation', {}).get('enable')

                if current_enable == 'primaries':
                    self.logger.info("Found allocation.enable=primaries, fixing to enable replica allocation...")

                    # Enable full allocation
                    fix_cmd = [
                        'kubectl', 'exec', f'{cluster_name}-nodes-0', '-n', namespace, '--',
                        'curl', '-k', '-u', f'{username}:{password}',
                        '-X', 'PUT', '--silent', '--max-time', '10',
                        'https://localhost:9200/_cluster/settings',
                        '-H', 'Content-Type: application/json',
                        '-d', '{"transient":{"cluster.routing.allocation.enable":"all"}}'
                    ]

                    result = subprocess.run(fix_cmd, capture_output=True, text=True, timeout=15)
                    ...
                    if result.returncode == 0:
                        ...
                        if health_result.returncode == 0:
                            try:
                                ...
                                self.logger.info(f"Cluster health after allocation fix: {status}")
                            except json.JSONDecodeError:
                                self.logger.warning("Could not parse cluster health response")

                        return True
                    else:
                        self.logger.error(f"Failed to enable replica allocation: {result.stderr}")
                        return False

                else:
                    self.logger.info(f"Allocation setting is already correct: {current_enable}")
                    return True
            ...
            self.logger.error(f"Exception while fixing cluster allocation settings: {e}")
            return False
...
class UpdateClusterAllocationSettingsAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        if not security_config.get('enabled', False):
            return ActionResult(True, "Security is disabled, skipping allocation settings update")

        self.logger.info(f"Updating cluster allocation settings (enable: {allocation_enable})")
        ...
        try:
            ...
            with client:
                ...
                settings_data["transient"]["cluster.routing.allocation.enable"] = allocation_enable

                # Set disk watermarks if provided
                if disk_watermarks:
                    if 'low' in disk_watermarks:
                        settings_data["transient"]["cluster.routing.allocation.disk.watermark.low"] = disk_watermarks['low']
                    if 'high' in disk_watermarks:
                        settings_data["transient"]["cluster.routing.allocation.disk.watermark.high"] = disk_watermarks['high']
                    if 'flood_stage' in disk_watermarks:
                        settings_data["transient"]["cluster.routing.allocation.disk.watermark.flood_stage"] = disk_watermarks['flood_stage']

                # Set concurrent recoveries if provided
                if concurrent_recoveries is not None:
                    settings_data["transient"]["cluster.routing.allocation.node_concurrent_recoveries"] = concurrent_recoveries

                # Set recovery speed if provided
                if recovery_speed is not None:
                    settings_data["transient"]["indices.recovery.max_bytes_per_sec"] = recovery_speed

                # Set allow rebalance if provided
                if allow_rebalance is not None:
                    settings_data["transient"]["cluster.routing.rebalance.enable"] = allow_rebalance
                ...
                if client.update_cluster_settings(settings_data):
                    ...
                    current_enable = current_settings.get('transient', {}).get('cluster', {}).get('routing', {}).get('allocation', {}).get('enable')
                    self.logger.info(f"Verified allocation settings - enable: {current_enable}")

                    return ActionResult(True, f"Cluster allocation settings updated successfully (enable: {allocation_enable})")
                else:
                    return ActionResult(False, "Failed to update cluster allocation settings")

        except Exception as e:
            return ActionResult(False, f"Error updating cluster allocation settings: {e}")
...
class DeleteClusterAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            cmd = ['kubectl', 'delete', 'opensearchcluster', cluster_name, '-n', namespace]
            if force:
                cmd.append('--force')
            ...
            if result.returncode == 0:
                self.logger.info(f"OpenSearch cluster {cluster_name} deleted successfully")

                # Also clean up security secrets if they exist
                self._cleanup_security_secrets(namespace)

                return ActionResult(True, f"Cluster {cluster_name} deleted successfully")
            else:
            ...
            for secret_name in security_secrets:
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        self.logger.warning(f"Failed to delete secret {secret_name}: {result.stderr}")
