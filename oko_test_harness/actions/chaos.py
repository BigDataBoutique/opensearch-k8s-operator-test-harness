...
class InjectPodFailureAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            return ActionResult(
                True, 
                f"Injected {method} failure on {count} {target} pod(s)"
            )
...
class InjectNodeFailureAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        node_selector = params.get('node_selector', 'node-role.kubernetes.io/worker=true')
        count = params.get('count', 1)
        ...
        try:
            ...
            for node in target_nodes:
                if method == 'drain':
                    ...
                    return ActionResult(False, f"Unsupported node failure method: {method}")
            
            return ActionResult(
                True,
                f"Applied {method} to {count} node(s): {', '.join(target_nodes)}"
            )
...
class InjectNetworkPartitionAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        self.logger.info(f"Injecting network partition: {partition_type} {target} for {duration}")
    ...
    def _inject_with_chaos_mesh(self, target: str, duration: str, partition_type: str) -> ActionResult:
...
"""
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return ActionResult(False, f"Failed to inject network partition: {result.stderr}")
...
class InjectResourcePressureAction(BaseAction):
    ...
    def execute(self, params: Dict[str, Any]) -> ActionResult:
        ...
        self.logger.info(f"Injecting {resource} pressure on {target} (limit: {limit}, duration: {duration})")
        ...
        try:
            ...
            for pod in pods:
                self._apply_resource_pressure(pod['name'], namespace, resource, limit, duration)
            
            return ActionResult(
                True,
                f"Applied {resource} pressure to {len(pods)} {target} pod(s)"
            )
    ...
    def _apply_resource_pressure(self, pod_name: str, namespace: str, 
                               resource: str, limit: str, duration: str) -> bool:
        ...
        try:
            if resource == 'memory':
                ...
                cmd = [
                    'kubectl', 'exec', pod_name, '-n', namespace, '--',
                    'stress-ng', '--vm', '1', '--vm-bytes', limit, '--timeout', duration
                ]
            elif resource == 'cpu':
