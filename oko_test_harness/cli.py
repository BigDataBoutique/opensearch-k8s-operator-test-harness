...
@click.option(
    "--var-file", type=click.Path(exists=True), help="Variables file (JSON/YAML)"
)
@click.option(
    "--dry-run", is_flag=True, help="Parse and validate playbook without executing"
)
@click.option(
    "--continue-on-error", is_flag=True, help="Continue execution on step failures"
)
@click.pass_context
def run(
    ctx,
    ...
    if not playbook_path:
        ...
        if not playbook_dir.exists():
            logger.error(
                "No playbooks directory found. Run from the project root or specify a playbook path."
            )
            sys.exit(1)

        playbook_files = list(playbook_dir.glob("*.yaml")) + list(
            playbook_dir.glob("*.yml")
        )
        ...
        playbook_files.sort()
        logger.info(
            f"Found {len(playbook_files)} playbooks, running all sequentially..."
        )

        overall_success = True
        for playbook_file in playbook_files:
            logger.info(f"Running playbook: {playbook_file}")
            success = _run_single_playbook(
                ctx, playbook_file, var, var_file, dry_run, continue_on_error
            )
            if not success:
                overall_success = False
                if not continue_on_error:
                    logger.error(
                        f"Playbook {playbook_file} failed, stopping execution."
                    )
                    sys.exit(1)
                else:
                    logger.warning(
                        f"Playbook {playbook_file} failed, continuing due to --continue-on-error flag."
                    )
    ...
    success = _run_single_playbook(
        ctx, playbook_path, var, var_file, dry_run, continue_on_error
    )
    if not success:
        sys.exit(1)
...
def list_actions():
    ...
    categories = {
        ...
        "Scaling": ["scale_cluster", "scale_down_cluster"],
        "Upgrades": ["upgrade_cluster", "upgrade_operator"],
        "Chaos Engineering": [
            "inject_pod_failure",
            "inject_node_failure",
        ...
        "Monitoring": ["collect_logs", "capture_metrics", "debug_pause"],
    }
...
@cli.command()
@click.option(
    "--cluster-name", default="opensearch-test", help="Kubernetes cluster name"
)
...
@click.option(
    "--namespace", "-n", help="Specific namespace to delete (for existing clusters)"
)
@click.option(
    "--remove-operator",
    is_flag=True,
    help="Also remove the operator deployment and namespace (for existing clusters)",
)
def cleanup_all(
    force: bool,
    keep_docker: bool,
    ...
    if provider == "kind":
        ...
        if namespace:
            ...
            if remove_operator:
                ...
                click.echo(
                    "• Note: Operator deployment will be preserved (use --remove-operator to delete)"
                )
    else:
    ...
    if provider == "kind":
        ...
        success = success and _cleanup_existing_cluster_resources(
            namespace, remove_operator
        )
    else:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            for cluster in clusters:
                ...
                if delete_result.returncode == 0:
                    ...
                    logger.error(
                        f"Failed to delete cluster {cluster}: {delete_result.stderr}"
                    )
                    return False
        else:
            ...
            try:
                ...
                if profiles:
                    ...
                    for profile in profiles:
                        ...
                        logger.info(
                            f"Minikube profile: {profile_name} (status: {status})"
                        )
                        ...
                        if status == "Running":
                            ...
                            if stop_result.returncode == 0:
                                logger.info(
                                    f"Successfully stopped profile: {profile_name}"
                                )
                            else:
                                logger.warning(
                                    f"Failed to stop profile {profile_name}: {stop_result.stderr}"
                                )
                        else:
                ...
                logger.warning(
                    "Could not parse minikube profile list, trying fallback approach"
                )
                return True
        else:
...
def _cleanup_existing_cluster_resources(
    target_namespace: str = None, remove_operator: bool = False
) -> bool:
    ...
    try:
        ...
        try:
            ...
            config = parser.get_global_config()
            operator_namespace = (
                config.opensearch.operator_namespace or "opensearch-operator-system"
            )
        except Exception as e:
        ...
        if target_namespace:
            logger.info(
                f"Cleaning up OpenSearch resources in namespace: {target_namespace}"
            )
            ...
            try:
                ...
                if result.returncode != 0 and result.stderr:
                    logger.warning(
                        f"Error deleting OpenSearch resources: {result.stderr}"
                    )
            except subprocess.TimeoutExpired:
                logger.warning(
                    f"Timeout while cleaning up OpenSearch resources in {target_namespace}"
                )
            except Exception as e:
                logger.warning(
                    f"Error cleaning up OpenSearch resources in {target_namespace}: {e}"
                )
            ...
            if delete_result.returncode == 0:
                ...
                logger.warning(
                    f"Failed to delete namespace {target_namespace}: {delete_result.stderr}"
                )
        else:
            ...
            if ns_result.returncode == 0:
                ...
                for ns_line in namespaces:
                    ...
                    if "/" in ns_line:
                        ...
                        if ns_name.startswith("opensearch-"):
                            ...
                            is_operator_ns = (
                                ns_name == operator_namespace
                                or ns_name == "opensearch-operator-system"
                                or "operator" in ns_name
                            )

                            # Include operator namespaces only if remove_operator is True
                            if is_operator_ns and not remove_operator:
                                logger.info(
                                    f"Skipping operator namespace: {ns_name} (use --remove-operator to delete)"
                                )
                                continue
                ...
                if opensearch_namespaces:
                    logger.info(
                        f"Found {len(opensearch_namespaces)} opensearch namespaces to clean up: {', '.join(opensearch_namespaces)}"
                    )

                    # First delete OpenSearch custom resources in all namespaces to allow proper pod termination
                    logger.info(
                        "Deleting OpenSearch custom resources in all target namespaces..."
                    )
                    for namespace in opensearch_namespaces:
                        logger.info(
                            f"Deleting OpenSearch resources in namespace: {namespace}"
                        )
                        try:
                            # First try to remove finalizers from any opensearch clusters
                            clusters_result = subprocess.run(
                            ...
                            if (
                                clusters_result.returncode == 0
                                and clusters_result.stdout.strip()
                            ):
                                for (
                                    cluster_line
                                ) in clusters_result.stdout.strip().split("\n"):
                                    cluster_name = cluster_line.split("/")[-1]
                                    logger.info(
                                        f"Removing finalizers from {cluster_name}"
                                    )
                                    subprocess.run(
                                        [
                                            "kubectl",
                            ...
                            if result.returncode != 0 and result.stderr:
                                logger.warning(
                                    f"Error deleting OpenSearch resources in {namespace}: {result.stderr}"
                                )
                        except subprocess.TimeoutExpired:
                            logger.warning(
                                f"Timeout while cleaning up OpenSearch resources in {namespace}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Error cleaning up OpenSearch resources in {namespace}: {e}"
                            )
                    ...
                    for namespace in opensearch_namespaces:
                        ...
                        if delete_result.returncode == 0:
                            ...
                            logger.warning(
                                f"Failed to delete namespace {namespace}: {delete_result.stderr}"
                            )
                else:
        ...
        if remove_operator or target_namespace is None:
            if remove_operator:
                logger.info(
                    "Cleaning up OpenSearch CRDs (operator removal requested)..."
                )
            else:
            ...
            logger.info(
                "Skipping CRD and cluster role cleanup (specific namespace targeted)"
            )
        ...
        if list_result.returncode == 0 and list_result.stdout.strip():
            ...
            for release_line in opensearch_releases:
                ...
                if len(parts) >= 2:
                    ...
                    namespace = parts[1]
                    logger.info(
                        f"Uninstalling Helm release: {release_name} (namespace: {namespace})"
                    )
                    ...
                    if uninstall_result.returncode == 0:
                        ...
                        logger.warning(
                            f"Failed to uninstall {release_name}: {uninstall_result.stderr}"
                        )
    ...
    try:
        ...
        if contexts_result.returncode == 0:
            ...
            if provider == "kind":
                ...
                for context in kind_contexts:
                    ...
                    if check_result.returncode == 0:
                        ...
                        if cluster_name not in existing_clusters:
                            ...
                            subprocess.run(
                                ["kubectl", "config", "unset", f"users.{context}"],
                                capture_output=True,
                            )
                ...
                logger.info(
                    "Preserving minikube kubectl contexts (clusters not deleted)"
                )
