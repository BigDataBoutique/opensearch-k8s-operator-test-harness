...
class PlaybookParser:
    ...
    def _merge_configs(self, global_config: Dict[str, Any], playbook_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _apply_default_namespace(self, data: Dict[str, Any]) -> None:
        ...
        if not current_namespace:
            ...
            logger.debug(f"Applied random operator_namespace: {self._default_namespace}")
        ...
        for phase in phases:
            for step in phase.get('steps', []):
                ...
                if 'namespace' in params and params['namespace'] in ['opensearch', 'opensearch-system']:
                    params['namespace'] = self._default_namespace
                    logger.debug(f"Updated step namespace to: {self._default_namespace}")
    ...
    def _apply_default_cluster_name(self, data: Dict[str, Any]) -> None:
        ...
        elif 'cluster_name' in opensearch_config and opensearch_config['cluster_name'] != 'test-cluster':
        ...
        try:
            ...
            for phase in playbook.phases:
                ...
                for step in phase.steps:
                    if not step.action:
                        logger.error(f"Step in phase '{phase.name}' must have an action")
                        return False
