...
class KubernetesManager:
    ...
    def apply_yaml_manifest(
        self, yaml_content: str, namespace: Optional[str] = None
    ) -> bool:
            ...
            return [
                {
                    ...
                    "crash_loop_backoff": any(
                        c.state
                        and c.state.waiting
                        and c.state.waiting.reason == "CrashLoopBackOff"
            ...
            if result.returncode == 0:
                ...
                if len(lines) > 1:
                    ...
                    available_mb = (
                        ...
                        else int(memory_line[3])
                    )
                    if (
                        available_mb < 6144
                    ):  # 6GB minimum (4GB for cluster + 2GB buffer)
                        logger.warning(
                            f"Low available memory: {available_mb}MB (recommended: >6144MB)"
                        )
                        return False
    ...
    def create_cluster(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"Creating Minikube cluster: {cluster_name} (attempt {attempt + 1}/{max_retries})"
                )
                ...
                if result.returncode == 0:
                    ...
                    if attempt < max_retries - 1:
                        logger.info("Retrying cluster creation in 10 seconds...")
                        time.sleep(10)

            except subprocess.TimeoutExpired:
                logger.error(
                    f"Timeout creating Minikube cluster (attempt {attempt + 1}/{max_retries})"
                )
                ...
                logger.error(
                    f"Error creating Minikube cluster (attempt {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    logger.info("Retrying cluster creation in 10 seconds...")
                    time.sleep(10)
...
class KindClusterManager:
    ...
    def _prepare_docker_space(self) -> None:
        ...
        try:
            ...
            subprocess.run(
                ["docker", "image", "prune", "-a", "-f"], capture_output=True, text=True
            )
            subprocess.run(
    ...
    def create_cluster(
        self, cluster_name: str, config_content: Optional[str] = None
    ) -> bool:
