...
class OpenSearchClient:
    ...
    def create_index(
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                logger.info(
                    f"Created index '{index_name}' with {shards} shard(s) and {replicas} replica(s)"
                )
                return True
            else:
        ...
        try:
            if doc_id:
                response = self.session.put(
                    f"{self.base_url}/{index_name}/_doc/{doc_id}", json=document
                )
            else:
                response = self.session.post(
                    f"{self.base_url}/{index_name}/_doc", json=document
                )
    ...
    def bulk_index(
        self, index_name: str, documents: List[Dict[str, Any]], chunk_size: int = 100
    ) -> Tuple[int, int]:
    ...
    def update_index_replicas(self, index_name: str, replicas: int) -> bool:
        ...
        try:
            settings = {"settings": {"number_of_replicas": replicas}}
            response = self.session.put(
                f"{self.base_url}/{index_name}/_settings", json=settings
            )
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to update replicas for {index_name}: {response.text}"
                )
                return False
        except Exception as e:
    ...
    def search(
        ...
        response = self.session.post(
            f"{self.base_url}/{index_name}/_search", json=search_body
        )
        response.raise_for_status()
        return response.json()
    ...
    def count(self, index_name: str, query: Optional[Dict[str, Any]] = None) -> int:
        ...
        response = self.session.post(f"{self.base_url}/{index_name}/_count", json=body)
        response.raise_for_status()
        return response.json()["count"]

    def wait_for_cluster_health(
        self, status: str = "yellow", timeout: int = 300
    ) -> bool:
        ...
        try:
            health = self.health()
            logger.error(
                f"Timeout waiting for cluster health {status}. Final status: {health['status']}"
            )
            logger.error(
                f"Final cluster state - Nodes: {health.get('number_of_nodes', 0)}, "
                f"Active shards: {health.get('active_shards', 0)}, "
                f"Unassigned shards: {health.get('unassigned_shards', 0)}"
            )
        except Exception as e:
            logger.error(
                f"Timeout waiting for cluster health {status}. Could not get final status: {e}"
            )
...
class KubernetesOpenSearchClient(OpenSearchClient):
    ...
    def __init__(
        self,
        namespace: str = "opensearch",
        service_name: str = "opensearch-cluster",
        port: int = 9200,
        username: str = "admin",
        password: str = "Admin123!",
        use_ssl: bool = None,
